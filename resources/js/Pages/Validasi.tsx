...
Validasi.layout = (page: React.ReactNode) => <AppLayout children={page} />;

export default function Validasi({ dokumens }: ValidasiProps) {
  ...
  const confirmApprove = () => {
    if (approveDoc) {
      approveForm.post(`/validasi/${approveDoc}/setujui`, {
  ...
  const handleRevisi = (e: React.FormEvent) => {
    e.preventDefault();
    if (revisiDoc) {
      revisiForm.post(`/validasi/${revisiDoc}/revisi`, {
        onSuccess: () => {
          setRevisiDoc(null);
          revisiForm.reset();
        }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-[#0f1923]">Validasi Dokumen</h1>
        <p className="text-slate-500 text-sm mt-1">
          Antrean dokumen dari Panitia yang berada pada fase <FaseBadge status="menunggu_validasi" size="sm" className="mx-1" />
          Periksa dan setujui untuk meneruskan ke Divisi IDE.
        </p>
      </div>
      ...
      >
        {dokumens.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-center px-4">
            ...
            <h3 className="text-slate-700 font-semibold text-lg">Tidak Ada Antrean</h3>
            <p className="text-slate-500 text-sm mt-1">
              Semua dokumen Draft telah divalidasi.
            </p>
          </div>
        ) : (
        ...
        <DialogContent className="sm:max-w-[425px]">
          <form onSubmit={handleRevisi}>
            <DialogHeader>
              ...
              <DialogDescription>
                Berikan catatan perbaikan yang harus dilakukan oleh panitia sebelum dokumen dapat divalidasi.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Misal: Format penulisan tanggal salah, harap disesuaikan dengan SOP..."
      ...
      <AlertDialog open={approveDoc !== null} onOpenChange={(open) => !open && setApproveDoc(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Yakin ingin menyetujui dokumen ini? Dokumen akan berpindah ke fase Validasi dan menunggu review dari Divisi IDE.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Batal</AlertDialogCancel>
            <AlertDialogAction onClick={confirmApprove} className="bg-teal-600 hover:bg-teal-700 text-white">
